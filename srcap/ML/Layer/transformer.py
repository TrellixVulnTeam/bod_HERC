...
class Model(BertModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config
        self.embeddings = GBST(num_tokens=config.vocab_size, dim=config.hidden_size,
                               max_block_size=4, downsample_factor=4, score_consensus_attn=True)
    ...
    def forward(
        ...
        embedding_output, extended_attention_mask = self.embeddings(
