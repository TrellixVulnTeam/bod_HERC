...
class GBST(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert exists(max_block_size) ^ exists(
            blocks), 'either max_block_size or blocks are given on initialization'
        self.token_emb = nn.Embedding(num_tokens, dim)

        if exists(blocks):
            assert isinstance(
                blocks, tuple), 'blocks must be a tuple of block sizes'
            self.blocks = tuple(
                map(lambda el: el if isinstance(el, tuple) else (el, 0), blocks))
            assert all([(offset < block_size) for block_size, offset in self.blocks]
                       ), 'offset must be always smaller than the block size'

            max_block_size = max(list(map(lambda t: t[0], self.blocks)))
        else:
            self.blocks = tuple(
                map(lambda el: (el, 0), range(1, max_block_size + 1)))
        ...
        self.block_pad_multiple = lcm(
            *[block_size for block_size, _ in self.blocks])
        self.downsample_factor = downsample_factor

    def forward(self, x, mask=None):
        b, n, block_mult, ds_factor, device = * \
            x.shape, self.block_pad_multiple, self.downsample_factor, x.device
        ...
        if exists(mask):
            mask = pad_to_multiple(
                mask, block_mult, seq_dim=1, dim=-1, value=False)
        ...
        for block_size, offset in self.blocks:
            ...
            if need_padding:
                ...
                block_x = F.pad(
                    block_x, (0, 0, left_offset, right_offset), value=0.)

                if exists(mask):
                    block_mask = F.pad(
                        block_mask, (left_offset, right_offset), value=False)
            ...
            if exists(mask):
                mask_blocks = rearrange(
                    block_mask, 'b (n m) -> b n m', m=block_size)
            ...
            if exists(mask):
                ...
                mask_blocks = repeat(
                    mask_blocks, 'b n -> b (n m)', m=block_size)
