...
class GBST_Extended(nn.Module):
    def __init__(self,num_tokens,num_tokens_lang,num_tokens_type,dim,max_block_size = None,blocks = None,downsample_factor = 4,score_consensus_attn = True):
        super().__init__()
        assert exists(max_block_size) ^ exists(blocks), 'either max_block_size or blocks are given on initialization'
        self.token_emb = nn.Embedding(num_tokens, dim)
        self.token_lang = nn.Embedding(num_tokens_lang, dim)
        self.token_type = nn.Embedding(num_tokens_type, dim)
    ...
    def forward(self, char,lang,type, mask = None):
        b, n, block_mult, ds_factor, device = *char.shape, self.block_pad_multiple, self.downsample_factor, char.device
        m = next_divisible_length(n, ds_factor)

        # get character token embeddings
        lang = self.token_lang(lang)
        type = self.token_type(type)
        char = self.token_emb(char)
        char = char + type + lang
        # do a conv to generate the positions for the tokens

        char = self.pos_conv(char)

        # pad both sequence and mask to length visibile by all block sizes from 0 to max block size

        char = pad_to_multiple(char, block_mult, seq_dim = 1, dim = -2)
        ...
        char = (block_reprs * scores).sum(dim = 2)
        ...
        char = rearrange(char, 'b (n m) d -> b n m d', m = ds_factor)

        if exists(mask):
            mask = rearrange(mask, 'b (n m) -> b n m', m = ds_factor)
            char = masked_mean(char, mask, dim = 2)
            mask = torch.any(mask, dim = -1)
        else:
            char = char.mean(dim = -2)
