...
async def url_add(url_from, where, item=None, item_type="website", dectect_item_type=True, Ps={}, d=True, p=False,session=None):
    try:
            ...
            if (mine == " ") and (mine is None) or "html" in mine:
                ...
                if check_html and item_type != "feed":
                    for feed in feeds:
                        await url_add(feed["href"], "wikidata",  item_type="feed", p=True,session=session)
            else:
                check_html = False
