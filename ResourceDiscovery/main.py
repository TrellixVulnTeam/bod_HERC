...
async def get_wikidata(name, prop, quarry, lock_SPRQL, type_="unknown", value_name=None):
    ...
    count = 0
    pending = []
    with Bar('Processing '+name, max=0) as bar_done:
        while True:
            async for data_i, size in SPRQL_GEN(quarry, sprql_endpoint, name+".json", lock_SPRQL):
                ...
                async def do_thing(datas, name="", prop="", sem=None, type_=None, value_name=None):
                    pending2 = []
                ...
                if len(things) > 4500:
                    await do_thing(things, name=name, sem=sem, prop=prop, type_=type_, value_name=value_name)
                    things = []
                if len(pending) != 0:
