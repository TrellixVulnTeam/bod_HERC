                           ...
                           qxl_async_io async)
{
    ...
    {
        qxl_set_guest_bug(d, "%s: guest_start > guest_end 0x%" PRIx64 " > 0x%" PRIx64, __func__, guest_start, guest_end);
        return 1;
    }
                                     ...
                                     qxl_async_io async)
{
    ...
    if (requested_stride == INT32_MIN ||
        abs(requested_stride) * (uint64_t)requested_height > qxl->vgamem_size)
    {
...
static const VMStateDescription qxl_vmstate = {
    ...
    .fields = (VMStateField[]){
        ...
        VMSTATE_END_OF_LIST()},
    .subsections = (const VMStateDescription *[]){&qxl_vmstate_monitors_config, NULL}};
