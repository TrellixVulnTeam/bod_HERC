...
#define SCR_REG_W(N)                                    \
    case SCR_##N:                                       \
        s->scr.N = (s->scr.N & ~0xff) | (value & 0xff); \
        return;                                         \
    case SCR_##N + 1:                                   \
        s->scr.N = (s->scr.N & 0xff) | (value << 8);    \
        return
#define SCR_REG_L(N)                                                    \
    case SCR_##N:                                                       \
        s->scr.N = (s->scr.N & ~0xff) | (value & 0xff);                 \
        return;                                                         \
    case SCR_##N + 1:                                                   \
        s->scr.N = (s->scr.N & ~(0xff << 8)) | (value & (0xff << 8));   \
        return;                                                         \
    case SCR_##N + 2:                                                   \
        s->scr.N = (s->scr.N & ~(0xff << 16)) | (value & (0xff << 16)); \
        return;                                                         \
    case SCR_##N + 3:                                                   \
        s->scr.N = (s->scr.N & ~(0xff << 24)) | (value & (0xff << 24)); \
        return;
#define SCR_REG_A(N)         \
    case SCR_##N(0):         \
        s->scr.N[0] = value; \
